...
def check_returned_data_entity(entity: str, looking_for: str):
    ...
    for index, row in df.iterrows():
        ...
        if int(row[f"available_{looking_for}"]) > int(row[f"fetched_{looking_for}"]):
            ...
            extract_sub_entity_from_entity(
                entity_id, row[f"list_of_{looking_for}"], looking_for, entity
            )
    ...
    if ids:
        ...
        upload_file(
            f"data/{entity}s_ids_for_{looking_for}_ingestion-final.txt", stream_file
        )
...
def extract_from_ingested_characters_comics_data(limit):
    ...
    for row in data[index:]:
        ...
        if int(row[4]) > int(row[5]):
            ...
            comics = ingest_comics_from_entity(
                http=http, entity_id=char_id, offset=0, limit=limit, entity="characters"
            )
            comics_from_characters_simplified = [
                simplify_comics_from_characters(char_id, y) for x in comics for y in x
            ]
    ...
    upload_file(
        "data/characters-in-comics/characters-in-comics-{date}.csv".format(
...
def extract_from_ingested_characters_events_data(limit):
    ...
    for row in data[index:]:
        ...
        if int(row[7]) > int(row[8]):
            # data_row = extract_and_save_comics_from_characters(entity_id, 100)
            comics = ingest_events_from_characters(
                http=http, char_id=char_id, offset=0, limit=limit
            )
            events_from_characters_simplified = [
                simplify_events_from_characters(char_id, y) for x in comics for y in x
            ]
    ...
    main_output.write("".join([x for x in outputs]))
    upload_file(
        "data/characters-in-events/characters-in-events-{date}.csv".format(
            date=str(datetime.now()).replace(" ", "-")
        ),
        main_output.getvalue(),
    )
...
def extract_from_ingested_creators_comics_data(limit):
    ...
    for row in data[index:]:
        ...
        if int(row[7]) > int(row[8]):
            ...
            comics_from_creators_simplified = [
                simplify_comics_from_creators(creator_id, y) for x in comics for y in x
            ]
    ...
    main_output.write("".join([x for x in outputs]))
    upload_file(
        "data/creators-in-comics/creators-in-comics-{date}.csv".format(
            date=str(datetime.now()).replace(" ", "-")
        ),
        main_output.getvalue(),
    )
