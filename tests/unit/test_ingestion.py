...
def test_ingest_entity_returns_true_for_another_request_when_more_data_entries_available(
    response,
):
    # response = {"status_code": 200, "data": {"total": 10, "offset": 0, "count": 4}}
...
@pytest.mark.parametrize(
    "response", [{"status_code": 200, "data": {"total": 10, "offset": 0, "count": 10}}]
)
def test_ingest_entity_returns_false_for_another_request_when_no_more_data_entries_available(
    response,
):
    # response = {"status_code": 200, "data": {"total": 10, "offset": 0, "count": 4}}
    another_request = check_for_another_request(response)
...
def test_fetching_entity_raises_api_client_error_exception(
    ...
    assert (
        err.value.args[0] == f"Marvel API failed with a client error code {status_code}"
    )
...
def test_fetching_entity_raises_api_server_error_exception(
    http, modified_since, offset, order_by, url, status_code
):
    ...
    with pytest.raises(APIServerError) as err:
        with when(requests.Session).get(...).thenReturn(mock_object):
            fetch_entity(http, modified_since, offset, order_by, url)
    assert (
        err.value.args[0] == f"Marvel API failed with a server error code {status_code}"
    )
